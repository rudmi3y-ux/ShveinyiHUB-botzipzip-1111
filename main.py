...
async def callback_faq_prices(update, context):
    """FAQ: Цены"""
    await update.callback_query.answer()
    text = ("💰 *Примерные цены на популярные услуги:*\n\n"
...
async def admin_panel_command(update, context):
    """Команда /admin для показа админ-панели"""
    user_id = update.effective_user.id
...
async def log_all_updates(update: Update, context):
    ...
    if update.callback_query:
        logger.info(
            f"📥 CALLBACK RECEIVED: {update.callback_query.data} from user {user_id}"
        )
    elif update.message:
...
def main() -> None:
    ...
    # Обновленные админ-команды
    app.add_handler(CommandHandler(
        "admin", admin_panel_command))  # Используем нашу функцию
    ...
    # Обновленные обработчики callback для админ-панели
    app.add_handler(
    ...
    # Добавляем обработчики для изменения статуса и связи с клиентом
    app.add_handler(
    ...
    # Обработчик для изменения статуса из админ-панели (старая версия для совместимости)
    app.add_handler(
                ...
                try:
                    ...
                    for order in orders:
                        try:
                            ...
                            logger.error(
                                f"Failed to send review request for order {order_id}: {e}"
                            )
                except Exception as e:
...
def run_with_restart():
    ...
    logger.info(
        "⏳ Ожидание 15 секунд для завершения предыдущего экземпляра...")
    time.sleep(15)
        ...
        try:
            ...
            if 'conflict' in error_str or 'terminated by other' in error_str:
                ...
                if conflict_retries >= max_conflict_retries:
                    logger.critical(
                        "⚠️ Конфликт не разрешён после 10 попыток. Остановка.")
                    break
                wait_time = 60
                logger.warning(
                    f"⚠️ Конфликт с другим экземпляром. Попытка {conflict_retries}/{max_conflict_retries}. Ждём {wait_time} сек..."
                )
            ...
            if retry_count < max_retries:
                ...
                logger.critical(
                    "Достигнут лимит перезапусков. Бот остановлен.")
