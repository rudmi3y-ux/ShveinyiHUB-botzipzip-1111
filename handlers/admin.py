...
from utils.database import (get_statistics, get_all_orders, get_all_users,
                            get_spam_logs, set_admin, is_admin,
                            get_orders_by_status)
from keyboards import get_admin_main_menu, get_admin_orders_submenu, get_admin_back_menu, get_admin_order_detail_keyboard
import os
import logging
...
async def broadcast_send(update: Update,
    ...
    await update.message.reply_text(
        f"📤 Отправка рассылки {len(users)} пользователям...")
...
async def set_admin_command(update: Update,
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Could not send notification to new admin {new_admin_id}: {e}"
            )
...
async def admin_menu_callback(update: Update,
    ...
    # Добавляем обработку новых callback_data
    if data == "admin_orders_menu":
        # Показываем подменю управления заказами
    ...
    if data == "admin_stats_today":
        ...
        today_orders = [
            o for o in orders if o.created_at and o.created_at >= today
        ]

        new_count = len([o for o in today_orders if o.status == 'new'])
        in_progress = len(
            [o for o in today_orders if o.status == 'in_progress'])
    ...
    if data == "admin_stats_week":
        ...
        orders = get_all_orders(limit=500)
        week_orders = [
            o for o in orders if o.created_at and o.created_at >= week_ago
        ]

        new_count = len([o for o in week_orders if o.status == 'new'])
        in_progress = len(
            [o for o in week_orders if o.status == 'in_progress'])
    ...
    if data == "admin_clients":
        ...
        sorted_users = sorted(users,
                              key=lambda u: order_counts.get(u.user_id, 0),
                              reverse=True)
    ...
    # Обработка детального просмотра заказа
    ...
    if data in status_map:
        ...
        for order in orders[:10]:
            ...
            service_name = SERVICE_NAMES.get(order.service_type,
                                             order.service_type or 'Услуга')
            ...
            keyboard.append([
                InlineKeyboardButton(f"📦 {formatted_id}",
...
async def change_order_status(update: Update,
    ...
    data = query.data
    # data format: "status_{new_status}_{order_id}"
    parts = data.split("_")
    if len(parts) < 3:
        return
    ...
    if success:
        await query.answer(f"✅ Статус обновлен на {new_status}",
                           show_alert=True)
...
async def contact_client(update: Update,
    ...
    if not order.user_id:
        await query.answer("❌ У заказа нет привязанного пользователя",
                           show_alert=True)
        return

    # Показываем кнопку для связи
    keyboard = [[
        InlineKeyboardButton("✉️ Написать в Telegram",
                             url=f"tg://user?id={order.user_id}")
    ],
                [
                    InlineKeyboardButton(
                        "◀️ Назад", callback_data=f"admin_view_{order.id}")
                ]]
    ...
    await query.edit_message_text(
        ...
        f"📦 Заказ: #{order.id}\n\n"
        f"Нажмите кнопку ниже, чтобы написать клиенту в Telegram:",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown")
